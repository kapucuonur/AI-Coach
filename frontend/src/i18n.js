import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';
import LanguageDetector from 'i18next-browser-languagedetector';

const resources = {
    en: {
        translation: {
            "dashboard_title": "CoachOnur - AI Training Dashboard",
            "daily_intelligence": "Your daily training intelligence",
            "online": "Online",
            "logout": "Logout",
            "sync_reminder": "Note to athlete: Please sync your device with Garmin Connect to ensure I have your latest data!",
            "resting_hr": "Resting HR",
            "vo2_max": "VO2 Max",
            "stress": "Stress",
            "body_battery": "Body Battery",
            "sleep": "Sleep",
            "fitness_age": "Fitness Age",
            "coach_settings": "Coach Settings",
            "age": "Age",
            "gender": "Gender",
            "male": "Male",
            "female": "Female",
            "strength_training": "Strength Training (Days/Week): {{days}}",
            "off_days": "Off Day(s) - Rest Days",
            "training_plan_title": "Training Plan",
            "training_plan_desc": "Generate a structured plan tailored to your goals.",
            "generate_1w": "Generate 1-Week Plan",
            "generate_1m": "Generate 1-Month Plan",
            "recent_activities": "Recent Activities",
            "coach_daily_briefing": "Daily Briefing",
            "ai_performance_coach": "AI Performance Coach",
            "send_to_watch": "Send to Watch",
            "sending": "Sending...",
            "sent_to_watch": "Sent to Watch!",
            "failed": "Failed",
            "cancel": "Cancel",
            "save_settings": "Save Settings",
            "saving": "Saving...",
            "language": "Language",
            "performance_metrics": "Performance Metrics",
            "threshold_pace": "Threshold Pace (min/km)",
            "ftp": "FTP (Watts)",
            "max_power": "Max Power (Watts)",
            "swim_pace": "Swim Pace (min/100m)",
            "max_hr": "Max HR (bpm)",
            "primary_sport": "Primary Sport",
            "upcoming_races": "Upcoming Races",
            "add_race": "Add Race",
            "no_races": "No races added yet.",
            "race_name": "Race Name",
            "training_goals": "Training Goals",
            "running_target": "Running Target",
            "triathlon_distance": "Triathlon Distance",
            "cycling_focus": "Cycling Focus",
            "reset_plan": "Reset Plan",
            "week": "Week",
            "bpm": "bpm",
            "km": "km",
            "min": "min",
            "hrs": "hrs",
            "yrs": "yrs",
            "updating_data": "Updating data...",
            "monday": "Monday",
            "tuesday": "Tuesday",
            "wednesday": "Wednesday",
            "thursday": "Thursday",
            "friday": "Friday",
            "saturday": "Saturday",
            "sunday": "Sunday",
            "chat_greeting": "Hello! How are you feeling today? Any soreness, fatigue, or anything else I should know about? (Use the buttons on the dashboard to generate your training plan!)",
            "chat_placeholder": "Ask your coach...",
            "chat_conn_error": "Sorry, I'm having trouble connecting to the Coach Brain right now.",
            "also_runs": "Do you also run?",
            "also_runs_desc": "If unchecked, CoachOnur - AI Training won't prescribe running workouts.",
            "daily_training_time": "Daily Training Time",
            "daily_training_desc": "How much time do you have to train today?",
            "update_plan": "Update Plan",
            "upgrade_title": "Unlock AI Workouts & Deeper Insights",
            "upgrade_desc": "You are currently on the Free \"View-Only\" tier. Upgrade to Premium to generate personalized daily AI training plans, send them to your Garmin watch, and interact with your deeper fitness metrics.",
            "upgrade_button": "Upgrade to Premium",
            "admin_mode": "Admin Mode Active",
            "admin_mode_desc": "You bypass all paywalls for testing.",
            "premium_feature": "Premium Feature",
            "premium_feature_desc": "Upgrade to view and generate AI workouts.",
            "unlock_now": "Unlock Now",
            "trial_active_title": "1-Week Free Trial Active!",
            "trial_active_desc": "Your free trial ends soon. Subscribe now to ensure uninterrupted access.",
            "subscribe_now": "Subscribe Now",
            "check_your_plate": "CHECK YOUR PLATE",
            "todays_nutrition": "Today's Nutrition",
            "upload_food_photo": "Upload Food Photo",
            "analyzing_food": "Analyzing food...",
            "click_to_upload": "Click to upload",
            "or_drag_drop": "or drag and drop",
            "calories": "Calories",
            "protein": "Protein",
            "carbs": "Carbs",
            "fats": "Fats",
            "todays_meals": "Today's Meals",
            "yearly_progression": "Yearly Progression",
            "total_distance_elev": "Total distance (km) & Elevation (m) by sport",
            "total": "Total",
            "elevation": "Elevation",
            "m_elev": "m elev",
            "running": "Running",
            "cycling": "Cycling",
            "swimming": "Swimming",
            "strength_training": "Strength Training",
            "hiking": "Hiking",
            "walking": "Walking",
            "other": "Other",
            "ready_to_train": "Ready to train?",
            "hero_subtitle": "Your daily performance intelligence and AI coaching report is ready."
        }
    },
    tr: {
        translation: {
            "dashboard_title": "CoachOnur - AI Training Paneli",
            "daily_intelligence": "Günlük antrenman zekanız",
            "online": "Çevrimiçi",
            "logout": "Çıkış Yap",
            "sync_reminder": "Sporcuya not: Güncel verilerinizi alabilmem için lütfen cihazınızı Garmin Connect ile senkronize edin!",
            "resting_hr": "Dinlenik KH",
            "vo2_max": "VO2 Maks",
            "stress": "Stres",
            "body_battery": "Vücut Bataryası",
            "sleep": "Uyku",
            "fitness_age": "Kondisyon Yaşı",
            "coach_settings": "Koç Ayarları",
            "age": "Yaş",
            "gender": "Cinsiyet",
            "male": "Erkek",
            "female": "Kadın",
            "strength_training": "Kuvvet Antrenmanı (Gün/Hafta): {{days}}",
            "off_days": "Dinlenme Günleri",
            "training_plan_title": "Antrenman Programı",
            "training_plan_desc": "Hedeflerinize özel bir program oluşturun.",
            "generate_1w": "1 Haftalık Plan Oluştur",
            "generate_1m": "1 Aylık Plan Oluştur",
            "recent_activities": "Son Aktiviteler",
            "coach_daily_briefing": "Günlük Özet",
            "ai_performance_coach": "AI Performans Koçu",
            "send_to_watch": "Saate Gönder",
            "sending": "Gönderiliyor...",
            "sent_to_watch": "Saate Gönderildi!",
            "failed": "Hata",
            "cancel": "İptal",
            "save_settings": "Ayarları Kaydet",
            "saving": "Kaydediliyor...",
            "language": "Dil",
            "performance_metrics": "Performans Metrikleri",
            "threshold_pace": "Eşik Tempo (dk/km)",
            "ftp": "FTP (Watt)",
            "max_power": "Maks Güç (Watt)",
            "swim_pace": "Yüzme Temposu (dk/100m)",
            "max_hr": "Maks KH (bpm)",
            "primary_sport": "Ana Spor",
            "upcoming_races": "Gelecek Yarışlar",
            "add_race": "Yarış Ekle",
            "no_races": "Henüz yarış eklenmedi.",
            "race_name": "Yarış Adı",
            "training_goals": "Antrenman Hedefleri",
            "running_target": "Koşu Hedefi",
            "triathlon_distance": "Triatlon Mesafesi",
            "cycling_focus": "Bisiklet Odağı",
            "reset_plan": "Planı Sıfırla",
            "week": "Hafta",
            "bpm": "bpm",
            "km": "km",
            "min": "dk",
            "hrs": "sa",
            "yrs": "yaş",
            "updating_data": "Veriler güncelleniyor...",
            "monday": "Pazartesi",
            "tuesday": "Salı",
            "wednesday": "Çarşamba",
            "thursday": "Perşembe",
            "friday": "Cuma",
            "saturday": "Cumartesi",
            "sunday": "Pazar",
            "chat_greeting": "Merhaba! Bugün nasıl hissediyorsun? Herhangi bir ağrı, yorgunluk veya bilmem gereken bir durum var mı? (Antrenman programı oluşturmak için panodaki butonları kullanabilirsin!)",
            "chat_placeholder": "Koçuna sor...",
            "chat_conn_error": "Üzgünüm, şu anda Coach Brain ile bağlantı kuramıyorum.",
            "also_runs": "Ayrıca koşuyor musunuz?",
            "also_runs_desc": "İşaretlenmezse, CoachOnur - AI Training koşu antrenmanları yazmayacaktır.",
            "daily_training_time": "Günlük Antrenman Süresi",
            "daily_training_desc": "Bugün antrenman için ne kadar vaktiniz var?",
            "update_plan": "Planı Güncelle",
            "upgrade_title": "AI Antrenmanlarını ve Derin Analizleri Açın",
            "upgrade_desc": "Şu anda Ücretsiz \"Sadece Görüntüleme\" paketindesiniz. Kişiselleştirilmiş günlük AI antrenman planları oluşturmak, bunları Garmin saatinize göndermek ve daha derin fitness metriklerinizle etkileşime geçmek için Premium'a yükseltin.",
            "upgrade_button": "Premium'a Yükselt",
            "admin_mode": "Yönetici Modu Aktif",
            "admin_mode_desc": "Test için tüm ödeme duvarlarını atlarsınız.",
            "premium_feature": "Premium Özellik",
            "premium_feature_desc": "AI antrenmanlarını görmek ve oluşturmak için yükseltin.",
            "unlock_now": "Şimdi Aç",
            "trial_active_title": "1 Haftalık Ücretsiz Deneme Aktif!",
            "trial_active_desc": "Ücretsiz deneme süreniz yakında bitiyor. Kesintisiz erişim için şimdi abone olun.",
            "subscribe_now": "Şimdi Abone Ol",
            "check_your_plate": "TABAĞINI KONTROL ET",
            "todays_nutrition": "Bugünün Beslenmesi",
            "upload_food_photo": "Yemek Fotoğrafı Yükle",
            "analyzing_food": "Yemek analiz ediliyor...",
            "click_to_upload": "Yüklemek için tıklayın",
            "or_drag_drop": "veya sürükleyip bırakın",
            "calories": "Kalori",
            "protein": "Protein",
            "carbs": "Karbonhidrat",
            "fats": "Yağlar",
            "todays_meals": "Bugünün Öğünleri",
            "yearly_progression": "Yıllık Gelişim",
            "total_distance_elev": "Spora Göre Toplam Mesafe (km) & Yükseklik (m)",
            "total": "Toplam",
            "elevation": "Yükseklik",
            "m_elev": "m yükseklik",
            "running": "Koşu",
            "cycling": "Bisiklet",
            "swimming": "Yüzme",
            "strength_training": "Kuvvet Antrenmanı",
            "hiking": "Doğa Yürüyüşü",
            "walking": "Yürüyüş",
            "other": "Diğer",
            "ready_to_train": "Sınırlarını zorlamaya hazır mısın?",
            "hero_subtitle": "Günlük performans zekan ve yapay zeka koçluk raporun hazır."
        }
    },
    de: {
        translation: {
            "dashboard_title": "CoachOnur - AI Training Dashboard",
            "daily_intelligence": "Ihre tägliche Trainingsintelligenz",
            "online": "Online",
            "logout": "Abmelden",
            "sync_reminder": "Hinweis an den Athleten: Bitte synchronisieren Sie Ihr Gerät mit Garmin Connect!",
            "resting_hr": "Ruhepuls",
            "vo2_max": "VO2 Max",
            "stress": "Stress",
            "body_battery": "Body Battery",
            "sleep": "Schlaf",
            "fitness_age": "Fitnessalter",
            "coach_settings": "Coach-Einstellungen",
            "recent_activities": "Neueste Aktivitäten",
            "coach_daily_briefing": "Tägliches Briefing",
            "send_to_watch": "An Uhr senden",
            "language": "Sprache",
            "also_runs": "Laufen Sie auch?",
            "also_runs_desc": "Falls nicht markiert, verschreibt CoachOnur - AI Training keine Lauftrainings.",
            "daily_training_time": "Tägliche Trainingszeit",
            "daily_training_desc": "Wie viel Zeit haben Sie heute für das Training?",
            "update_plan": "Plan aktualisieren",
            "upgrade_title": "Schalten Sie KI-Workouts und tiefere Einblicke frei",
            "upgrade_desc": "Sie befinden sich derzeit in der kostenlosen \"Nur-Lese\"-Stufe. Führen Sie ein Upgrade auf Premium durch, um personalisierte tägliche KI-Trainingspläne zu erstellen, diese an Ihre Garmin-Uhr zu senden und mit Ihren tiefergehenden Fitnessmetriken zu interagieren.",
            "upgrade_button": "Auf Premium upgraden",
            "admin_mode": "Admin-Modus aktiv",
            "admin_mode_desc": "Sie umgehen alle Paywalls zu Testzwecken.",
            "premium_feature": "Premium-Funktion",
            "premium_feature_desc": "Führen Sie ein Upgrade durch, um KI-Workouts anzuzeigen und zu generieren.",
            "unlock_now": "Jetzt freischalten",
            "chat_greeting": "Hallo! Wie fühlst du dich heute? Hast du Muskelkater, Müdigkeit oder gibt es etwas anderes, das ich wissen sollte?",
            "trial_active_title": "1 Woche kostenlose Testversion aktiv!",
            "trial_active_desc": "Ihre kostenlose Testversion endet bald. Abonnieren Sie jetzt, um ununterbrochenen Zugang zu erhalten.",
            "subscribe_now": "Jetzt abonnieren"
        }
    },
    fr: {
        translation: {
            "dashboard_title": "Tableau de Bord CoachOnur - AI Training",
            "daily_intelligence": "Votre intelligence d'entraînement quotidienne",
            "online": "En ligne",
            "logout": "Déconnexion",
            "sync_reminder": "Note à l'athlète : Veuillez synchroniser votre appareil avec Garmin Connect !",
            "resting_hr": "FC au repos",
            "vo2_max": "VO2 Max",
            "stress": "Stress",
            "body_battery": "Body Battery",
            "sleep": "Sommeil",
            "fitness_age": "Âge de forme",
            "coach_settings": "Paramètres du Coach",
            "recent_activities": "Activités récentes",
            "coach_daily_briefing": "Briefing quotidien",
            "send_to_watch": "Envoyer à la montre",
            "language": "Langue",
            "also_runs": "Courez-vous aussi ?",
            "also_runs_desc": "Si décoché, CoachOnur - AI Training ne prescrira pas d'entraînements de course à pied.",
            "daily_training_time": "Temps d'entraînement quotidien",
            "daily_training_desc": "Combien de temps avez-vous pour vous entraîner aujourd'hui ?",
            "update_plan": "Mettre à jour le plan",
            "upgrade_title": "Débloquez les entraînements IA et des informations plus approfondies",
            "upgrade_desc": "Vous êtes actuellement sur le niveau gratuit \"Vue seule\". Passez à Premium pour générer des plans d'entraînement IA quotidiens personnalisés, les envoyer à votre montre Garmin et interagir avec vos mesures de forme physique plus approfondies.",
            "upgrade_button": "Passer à Premium",
            "admin_mode": "Mode Administrateur actif",
            "admin_mode_desc": "Vous contournez tous les murs de paiement pour tester.",
            "premium_feature": "Fonctionnalité Premium",
            "premium_feature_desc": "Passez à la version supérieure pour afficher et générer des entraînements IA.",
            "unlock_now": "Débloquer maintenant",
            "chat_greeting": "Salut ! Comment te sens-tu aujourd'hui ? Des douleurs, de la fatigue ou autre chose que je devrais savoir ?",
            "trial_active_title": "Essai gratuit d'une semaine actif !",
            "trial_active_desc": "Votre essai gratuit se termine bientôt. Abonnez-vous maintenant pour garantir un accès ininterrompu.",
            "subscribe_now": "S'abonner maintenant"
        }
    },
    it: {
        translation: {
            "dashboard_title": "Dashboard CoachOnur - AI Training",
            "daily_intelligence": "La tua intelligenza di allenamento quotidiana",
            "online": "Online",
            "logout": "Logout",
            "sync_reminder": "Nota per l'atleta: Sincronizza il tuo dispositivo con Garmin Connect!",
            "resting_hr": "FC a riposo",
            "vo2_max": "VO2 Max",
            "stress": "Stress",
            "body_battery": "Body Battery",
            "sleep": "Sonno",
            "fitness_age": "Età fitness",
            "coach_settings": "Impostazioni Coach",
            "recent_activities": "Attività recenti",
            "coach_daily_briefing": "Briefing giornaliero",
            "send_to_watch": "Invia all'orologio",
            "language": "Lingua",
            "also_runs": "Corri anche tu?",
            "also_runs_desc": "Se deselezionato, CoachOnur - AI Training non prescriverà allenamenti di corsa.",
            "daily_training_time": "Tempo di allenamento giornaliero",
            "daily_training_desc": "Quanto tempo hai a disposizione per allenarti oggi?",
            "update_plan": "Aggiorna il piano",
            "upgrade_title": "Sblocca gli allenamenti IA e approfondimenti",
            "upgrade_desc": "Attualmente sei sul livello gratuito \"Sola visualizzazione\". Passa a Premium per generare piani di allenamento IA giornalieri personalizzati, inviarli al tuo orologio Garmin e interagire con le tue metriche di fitness più approfondite.",
            "upgrade_button": "Passa a Premium",
            "admin_mode": "Modalità Amministratore attiva",
            "admin_mode_desc": "Ignora tutti i paywall per i test.",
            "premium_feature": "Funzione Premium",
            "premium_feature_desc": "Esegui l'upgrade per visualizzare e generare allenamenti IA.",
            "unlock_now": "Sblocca ora",
            "chat_greeting": "Ciao! Come ti senti oggi? Ci sono dolori, affaticamento o qualcos'altro che dovrei sapere?",
            "trial_active_title": "Prova gratuita di 1 settimana attiva!",
            "trial_active_desc": "La tua prova gratuita termina presto. Iscriviti ora per garantire un accesso ininterrotto.",
            "subscribe_now": "Iscriviti ora"
        }
    },
    es: {
        translation: {
            "dashboard_title": "Panel de Control CoachOnur - AI Training",
            "daily_intelligence": "Tu inteligencia de entrenamiento diaria",
            "online": "En línea",
            "logout": "Cerrar sesión",
            "sync_reminder": "Nota para el atleta: ¡Sincroniza tu dispositivo con Garmin Connect!",
            "resting_hr": "FC en reposo",
            "vo2_max": "VO2 Max",
            "stress": "Estrés",
            "body_battery": "Body Battery",
            "sleep": "Sueño",
            "fitness_age": "Edad de fitness",
            "coach_settings": "Ajustes del Entrenador",
            "recent_activities": "Actividades recientes",
            "coach_daily_briefing": "Resumen diario",
            "send_to_watch": "Enviar al reloj",
            "language": "Idioma",
            "also_runs": "¿También corres?",
            "also_runs_desc": "Si no está marcado, CoachOnur - AI Training no prescribirá entrenamientos de carrera.",
            "daily_training_time": "Tiempo de entrenamiento diario",
            "daily_training_desc": "¿Cuánto tiempo tienes para entrenar hoy?",
            "update_plan": "Actualizar plan",
            "upgrade_title": "Desbloquea entrenamientos de IA y métricas profundas",
            "upgrade_desc": "Actualmente estás en el nivel gratuito de \"Solo lectura\". Actualiza a Premium para generar planes de entrenamiento de IA diarios personalizados, enviarlos a tu reloj Garmin e interactuar con tus métricas de estado físico más profundas.",
            "upgrade_button": "Actualizar a Premium",
            "admin_mode": "Modo Administrador activo",
            "admin_mode_desc": "Ignoras todos los muros de pago para realizar pruebas.",
            "premium_feature": "Función Premium",
            "premium_feature_desc": "Actualiza para ver y generar entrenamientos de IA.",
            "unlock_now": "Desbloquear ahora",
            "chat_greeting": "¡Hola! ¿Cómo te sientes hoy? ¿Tienes dolores, fatiga o algo más que deba saber?",
            "trial_active_title": "¡Prueba gratuita de 1 semana activa!",
            "trial_active_desc": "Tu prueba gratuita termina pronto. Suscríbete ahora para garantizar un acceso ininterrumpido.",
            "subscribe_now": "Suscribirse ahora"
        }
    },
    ru: {
        translation: {
            "dashboard_title": "Панель управления CoachOnur - AI Training",
            "daily_intelligence": "Ваша ежедневная тренировочная аналитика",
            "online": "В сети",
            "logout": "Выйти",
            "sync_reminder": "Заметка для атлета: Пожалуйста, синхронизируйте устройство с Garmin Connect!",
            "resting_hr": "ЧСС в покое",
            "vo2_max": "VO2 Max",
            "stress": "Стресс",
            "body_battery": "Заряд тела",
            "sleep": "Сон",
            "fitness_age": "Спортивный возраст",
            "coach_settings": "Настройки коуча",
            "recent_activities": "Последние активности",
            "coach_daily_briefing": "Ежедневный брифинг",
            "send_to_watch": "Отправить на часы",
            "language": "Язык",
            "also_runs": "Вы также бегаете?",
            "also_runs_desc": "Если не отмечено, CoachOnur - AI Training не будет назначать беговые тренировки.",
            "daily_training_time": "Ежедневное время тренировки",
            "daily_training_desc": "Сколько у вас сегодня времени на тренировку?",
            "update_plan": "Обновить план",
            "upgrade_title": "Разблокируйте ИИ-тренировки и глубокую аналитику",
            "upgrade_desc": "В настоящее время вы находитесь на бесплатном уровне «Только просмотр». Перейдите на Premium, чтобы создавать персонализированные ежедневные планы ИИ-тренировок, отправлять их на часы Garmin и взаимодействовать с более глубокими показателями вашей физической формы.",
            "upgrade_button": "Перейти на Premium",
            "admin_mode": "Режим администратора активен",
            "admin_mode_desc": "Вы обходите все платные экраны для тестирования.",
            "premium_feature": "Премиум-функция",
            "premium_feature_desc": "Обновитесь для просмотра и создания ИИ-тренировок.",
            "unlock_now": "Разблокировать сейчас",
            "chat_greeting": "Привет! Как ты себя чувствуешь сегодня? Есть ли боль, усталость или что-то еще, что я должен знать?",
            "trial_active_title": "1 неделя бесплатной пробной версии активна!",
            "trial_active_desc": "Ваша бесплатная пробная версия скоро закончится. Подпишитесь сейчас, чтобы обеспечить бесперебойный доступ.",
            "subscribe_now": "Подписаться сейчас"
        }
    },
    fi: {
        translation: {
            "dashboard_title": "CoachOnur - AI Training Kojelauta",
            "daily_intelligence": "Päivittäinen harjoitusäly",
            "online": "Online",
            "logout": "Kirjaudu ulos",
            "sync_reminder": "Huomautus urheilijalle: Synkronoi laitteesi Garmin Connectin kanssa, jotta saan uusimmat tietosi!",
            "resting_hr": "Leposyke",
            "vo2_max": "VO2 Max",
            "stress": "Stressi",
            "body_battery": "Body Battery",
            "sleep": "Uni",
            "fitness_age": "Kuntoikä",
            "coach_settings": "Valmentajan asetukset",
            "age": "Ikä",
            "gender": "Sukupuoli",
            "male": "Mies",
            "female": "Nainen",
            "strength_training": "Voimaharjoittelu (Päiviä/viikko): {{days}}",
            "off_days": "Lepo-/vapaapäivät",
            "training_plan_title": "Harjoitussuunnitelma",
            "training_plan_desc": "Luo tavoitteidesi mukainen suunnitelma.",
            "generate_1w": "Luo 1 viikon suunnitelma",
            "generate_1m": "Luo 1 kuukauden suunnitelma",
            "recent_activities": "Viimeisimmät aktiviteetit",
            "coach_daily_briefing": "Päivittäinen katsaus",
            "ai_performance_coach": "Tekoälyvalmentaja",
            "send_to_watch": "Lähetä kelloon",
            "sending": "Lähetetään...",
            "sent_to_watch": "Lähetetty kelloon!",
            "failed": "Epäonnistui",
            "cancel": "Peruuta",
            "save_settings": "Tallenna asetukset",
            "saving": "Tallennetaan...",
            "language": "Kieli",
            "performance_metrics": "Suorituskykymittarit",
            "threshold_pace": "Kynnysvauhti (min/km)",
            "ftp": "FTP (Wattia)",
            "max_power": "Maksimiteho (Wattia)",
            "swim_pace": "Uintivauhti (min/100m)",
            "max_hr": "Maksimisyke (bpm)",
            "primary_sport": "Päälaji",
            "upcoming_races": "Tulevat kilpailut",
            "add_race": "Lisää kilpailu",
            "no_races": "Ei kilpailuja vielä.",
            "race_name": "Kilpailun nimi",
            "training_goals": "Harjoitustavoitteet",
            "running_target": "Juoksutavoite",
            "triathlon_distance": "Triathlon matka",
            "cycling_focus": "Pyöräilypainopiste",
            "reset_plan": "Nollaa suunnitelma",
            "week": "Viikko",
            "bpm": "bpm",
            "km": "km",
            "min": "min",
            "hrs": "tuntia",
            "yrs": "vuotta",
            "updating_data": "Päivitetään tietoja...",
            "monday": "Maanantai",
            "tuesday": "Tiistai",
            "wednesday": "Keskiviikko",
            "thursday": "Torstai",
            "friday": "Perjantai",
            "saturday": "Lauantai",
            "sunday": "Sunnuntai",
            "chat_greeting": "Hei! Miten voit tänään? Onko lihassärkyä, väsymystä tai jotain muuta, mitä minun pitäisi tietää? (Käytä kojelaudan painikkeita harjoitussuunnitelman luomiseen!)",
            "chat_placeholder": "Kysy valmentajaltasi...",
            "chat_conn_error": "Anteeksi, minulla on vaikeuksia yhdistää Coach Brainiin juuri nyt.",
            "also_runs": "Juoksetko myös?",
            "also_runs_desc": "Jos ei valittu, CoachOnur - AI Training ei määrää juoksuharjoituksia.",
            "daily_training_time": "Päivittäinen harjoitusaika",
            "daily_training_desc": "Paljonko sinulla on aikaa harjoitella tänään?",
            "update_plan": "Päivitä suunnitelma",
            "upgrade_title": "Avaa tekoälyharjoitukset ja syvällisemmät näkymät",
            "upgrade_desc": "Olet tällä hetkellä ilmaisella \"Vain luku\" -tasolla. Päivitä Premiumiin luodaksesi personoituja päivittäisiä tekoälyharjoitussuunnitelmia, lähettääksesi ne Garmin-kelloosi ja nähdäksesi syvällisemmät kuntomittarisi.",
            "upgrade_button": "Päivitä Premiumiin",
            "admin_mode": "Järjestelmänvalvojatila käytössä",
            "admin_mode_desc": "Ohitat kaikki maksumuurit testausta varten.",
            "premium_feature": "Premium-ominaisuus",
            "premium_feature_desc": "Päivitä nähdäksesi ja luodaksesi tekoälyharjoituksia.",
            "unlock_now": "Avaa nyt",
            "trial_active_title": "1 viikon ilmainen kokeilu käynnissä!",
            "trial_active_desc": "Ilmainen kokeilujaksosi päättyy pian. Tilaa nyt varmistaaksesi keskeytymättömän pääsyn.",
            "subscribe_now": "Tilaa nyt",
            "check_your_plate": "TARKISTA LAUTASESI",
            "todays_nutrition": "Tämän päivän ravinto",
            "upload_food_photo": "Lataa ruokakuva",
            "analyzing_food": "Analysoidaan ruokaa...",
            "click_to_upload": "Klikkaa ladataksesi",
            "or_drag_drop": "tai vedä ja pudota",
            "calories": "Kalorit",
            "protein": "Proteiinit",
            "carbs": "Hiilihydraatit",
            "fats": "Rasvat",
            "todays_meals": "Tämän päivän ateriat",
            "yearly_progression": "Vuotuinen kehitys",
            "total_distance_elev": "Kokonaispituus (km) & nousu (m) lajeittain",
            "total": "Yhteensä",
            "elevation": "Nousu",
            "m_elev": "m nousu",
            "running": "Juoksu",
            "cycling": "Pyöräily",
            "swimming": "Uinti",
            "strength_training": "Voimaharjoittelu",
            "hiking": "Vaellus",
            "walking": "Kävely",
            "other": "Muu",
            "ready_to_train": "Oletko valmis harjoittelemaan?",
            "hero_subtitle": "Päivittäinen suorituskykytietosi ja tekoälyvalmentajasi raportti ovat valmiina."
        }
    }
};

i18n
    .use(LanguageDetector)
    .use(initReactI18next)
    .init({
        resources,
        fallbackLng: 'en',
        interpolation: {
            escapeValue: false
        }
    });

export default i18n;
